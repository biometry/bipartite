...
\documentclass[a4paper, 11pt]{article}\usepackage[]{graphicx}\usepackage[dvipsnames]{xcolor}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# convert matrix into one-mode edgelist:}
\hlstd{SafPlantsEL} \hlkwb{<-} \hlkwd{as.tnet}\hlstd{(}\hlkwd{as.one.mode}\hlstd{(Safariland,} \hlkwc{project}\hlstd{=}\hlstr{"lower"}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in as.tnet(as.one.mode(Safariland, project = "{}lower"{})): Data assumed to be weighted one-mode tnet (if this is not correct, specify type)}}\end{kframe}
\end{knitrout}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# compute betweenness:}
\hlstd{tnet}\hlopt{::}\hlkwd{betweenness_w}\hlstd{(SafPlantsEL)} \hlcom{# 6}
\hlstd{bipartite}\hlopt{::}\hlkwd{BC}\hlstd{(Safariland,} \hlkwc{rescale}\hlstd{=F)}\hlopt{$}\hlstd{lower} \hlcom{# 6}
\hlstd{DiagrammeR}\hlopt{::}\hlkwd{get_betweenness}\hlstd{(DiagrammeR}\hlopt{::}\hlkwd{from_igraph}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL)))} \hlcom{# 6}
\hlstd{igraph}\hlopt{::}\hlkwd{betweenness}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL),} \hlkwc{cutoff}\hlstd{=}\hlnum{9}\hlstd{)} \hlcom{# 6}
\hlstd{influenceR}\hlopt{::}\hlkwd{betweenness}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL))} \hlcom{# 12}
\hlstd{sna}\hlopt{::}\hlkwd{betweenness}\hlstd{(SafPlantsEL)} \hlcom{# length 36!!}
\hlstd{sna}\hlopt{::}\hlkwd{betweenness}\hlstd{(}\hlkwd{as.matrix}\hlstd{(SafPlantsEL))} \hlcom{# length 27!!}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{btws} \hlkwb{<-} \hlkwd{cbind}\hlstd{(}\hlstr{"t:betw"}\hlstd{=tnet}\hlopt{::}\hlkwd{betweenness_w}\hlstd{(SafPlantsEL)[,}\hlnum{2}\hlstd{],}
    \hlstr{"b:BC"}\hlstd{=bipartite}\hlopt{::}\hlkwd{BC}\hlstd{(Safariland,} \hlkwc{rescale}\hlstd{=F)}\hlopt{$}\hlstd{lower,}
    \hlstr{"DR:betw"}\hlstd{=DiagrammeR}\hlopt{::}\hlkwd{get_betweenness}\hlstd{(DiagrammeR}\hlopt{::}\hlkwd{from_igraph}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL)))[,}\hlnum{2}\hlstd{],}
    \hlstr{"i:estbetw"}\hlstd{=igraph}\hlopt{::}\hlkwd{betweenness}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL),} \hlkwc{cutoff}\hlstd{=}\hlnum{9}\hlstd{),}
    \hlstr{"inf:betw"}\hlstd{=influenceR}\hlopt{::}\hlkwd{betweenness}\hlstd{(}\hlkwd{tnet_igraph}\hlstd{(SafPlantsEL))}\hlopt{/}\hlnum{2} \hlstd{)}
\hlkwd{rownames}\hlstd{(btws)} \hlkwb{<-} \hlkwd{rownames}\hlstd{(Safariland)}
\hlstd{btws}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{web.names} \hlkwb{<-} \hlkwd{data}\hlstd{(}\hlkwc{package}\hlstd{=}\hlstr{"bipartite"}\hlstd{)}\hlopt{$}\hlstd{results[,}\hlnum{3}\hlstd{]}
\hlkwd{data}\hlstd{(}\hlkwc{list}\hlstd{=web.names)} \hlcom{#loads all webs}
\hlcom{# the next step takes around 10 minutes:}
\hlstd{netw.indic.webs} \hlkwb{<-} \hlkwd{t}\hlstd{(}\hlkwd{sapply}\hlstd{(web.names,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{networklevel}\hlstd{(}\hlkwd{get}\hlstd{(x),}
              \hlkwc{index}\hlstd{=}\hlstr{"ALLBUTDD"}\hlstd{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{round}\hlstd{(PCA.out}\hlopt{$}\hlstd{rotation[,} \hlnum{1}\hlopt{:}\hlnum{4}\hlstd{],} \hlnum{3}\hlstd{)}
\end{alltt}
\begin{verbatim}
##                                         PC1    PC2    PC3    PC4
...
\begin{alltt}
\hlkwd{data}\hlstd{(Safariland)}
\hlstd{Iobs} \hlkwb{<-} \hlkwd{nestednodf}\hlstd{(Safariland)}\hlopt{$}\hlstd{statistic[}\hlnum{3}\hlstd{]}
\hlstd{nulls} \hlkwb{<-} \hlkwd{nullmodel}\hlstd{(}\hlkwc{web}\hlstd{=Safariland,} \hlkwc{N}\hlstd{=}\hlnum{1000}\hlstd{,} \hlkwc{method}\hlstd{=}\hlstr{'r2d'}\hlstd{)} \hlcom{# takes a while!}
\hlstd{Inulls} \hlkwb{<-} \hlkwd{sapply}\hlstd{(nulls,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{nestednodf}\hlstd{(x)}\hlopt{$}\hlstd{statistic[}\hlnum{3}\hlstd{])}
\hlkwd{plot}\hlstd{(}\hlkwd{density}\hlstd{(Inulls),} \hlkwc{xlim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{100}\hlstd{),} \hlkwc{lwd}\hlstd{=}\hlnum{2}\hlstd{,} \hlkwc{main}\hlstd{=}\hlstr{"NODF"}\hlstd{)}
\hlkwd{abline}\hlstd{(}\hlkwc{v}\hlstd{=Iobs,} \hlkwc{col}\hlstd{=}\hlstr{"red"}\hlstd{,} \hlkwc{lwd}\hlstd{=}\hlnum{2}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
