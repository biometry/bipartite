...
```{r, fig.width=10, fig.height=10, error=TRUE}
?plotweb_v2
use_deprecated_plotweb(F)
plotweb_v2 <- plotweb
plot2webs_v2 <- plot2webs
...
Many times we aren't just interested in visualizing a single web but rather want to show how a network changes with regards to a specific factor (e.g. time). At least for two webs this is now possible with the function `plot2webs_v2`. This function just takes two webs as input and generates the following plot.
...
As you can see both webs are plotted in one figure with the pollinator species (higher / columns) being aligned in the middle.

### Changing the middle

Sometimes you might be more interested in the interactions of the lower trophic level species (plants / rows). So to switch the focus onto them you can change the default option `middle = "higher"` to `middle = "lower"`.
...
The plot is now flipped inside out with the plant species being aligned in the middle.

### Switching to a relative scaling

Similarly to the total abundances being different on each side of a single plot the total abundances of each plot in `plot2webs_v2` can differ as well. However here the default is to use the absolute scaling so you can compare the total change of abundances. If you're only interested in the relative change you might use the option `scaling = "relative"`.
...
Now we are still especially interested in the interactions of the plant species *Alstroemeria aurea* so we use the `lower_color` vector from above together with the `link_color = "lower"` argument to highlight their interactions in both webs.
...
However it would be even nicer if we had our species of interest in the middle of the plot so we just set `middle = "lower"`.
...
Now the error tells us that our `lower_color` vector doesn't contain all the lower species present in the plot. That is because the `vazarr` data set contains some plants which are not in `Safariland`. So to fix this we just need to generate a new vector `combined_lower_color` that contains all the lower trophic species names in the plot.
...
We also set the option `curved_links = TRUE`.
